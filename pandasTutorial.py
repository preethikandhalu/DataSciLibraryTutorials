...
#INSPECTION
info = movies.info()
m_type = movies.dtypes      #just for data types of each column
stat2 = users.describe()    #quick stats. return information on all columns of a numeric datatype.

#to see particular rows
slic = movies[20:22]
slic2 = movies[20:21]       #for just row 20

#SELECTING
#Selecting columns
occ=users['occupation'].head()
occ2=users[['occupation', 'sex']].head()

#row selection
#users older than 25
sel1=users[users.age > 25].head()

#users aged 40 and male
sel2=users[(users.age==40) & (users.sex=='M')].head()

#users younger than 20 or female
sel3=users[(users.sex=='F') | (users.age < 30)].head()

#set_index()
"""
Since our index is kind of meaningless right now, let's set it to the user_id
using the set_index method. By default, set_index returns a new DataFrame,
so you'll have to specify if you'd like the changes to occur in place.
"""
retr=users.set_index('user_id').head()  #return new DataFrame
users2=users.head(500)
reti=users2.set_index('user_id', inplace='True')    #to do inplace. reti is None

#users2 has index set with user_id. users is the original
#select row BY POSITION
iloc1=users2.iloc[99]
iloc2=users2.iloc[[1,50,300]]

#select row BY LABEL
loc1=users2.loc[99]
loc2=users2.loc[[1,50,300]]

#if we want to get the old pandas index back, use
#users.reset_index()
#same rules for inplace apply
...
join=pd.merge(left_frame, right_frame, on='key')

#see resource for left, right and outer join

#####################COMBINING
#equivalent to SQL's UNION
#look at resource for more info

#####################GROUPING
#equiv to SQL's GROUp BY
#FOLLOW RESOURCE!!!
